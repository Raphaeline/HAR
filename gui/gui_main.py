...
def get_trackColors(n):
    ...
    for i in range(n):
        ...
        # Otherwise, generate a color from the average of two randomly selected colors, and add the new color to the list
            ...
            (r_2, g_2, b_2, _) = modKellyColorsNorm[random.randint(
                0, len(modKellyColorsNorm)-1)]
            (r_1, g_1, b_1, _) = modKellyColorsNorm[random.randint(
                0, len(modKellyColorsNorm)-1)]
            r_gen = (r_2 + r_1) / 2
            g_gen = (g_2 + g_1) / 2
            b_gen = (b_2 + b_1) / 2
...
class Window(QDialog):
    def __init__(self, parent=None, size=[]):
        ...
        if (0):  # set to 1 to save terminal output to logFile, set 0 to show terminal output
            ts = time.localtime()
            terminalFileName = str('logData/logfile_' + str(ts[2]) + str(
                ts[1]) + str(ts[0]) + '_' + str(ts[3]) + str(ts[4]) + '.txt')
        ...
        self.profile = {'startFreq': 60.25, 'numLoops': 64, 'numTx': 3,
                        'sensorHeight': 3, 'maxRange': 10, 'az_tilt': 0, 'elev_tilt': 0, 'enabled': 0}
        self.chirpComnCfg = {'DigOutputSampRate': 23, 'DigOutputBitsSel': 0, 'DfeFirSel': 0,
                             'NumOfAdcSamples': 128, 'ChirpTxMimoPatSel': 4, 'ChirpRampEndTime': 36.1, 'ChirpRxHpfSel': 1}
        self.chirpTimingCfg = {'ChirpIdleTime': 8, 'ChirpAdcSkipSamples': 24,
                               'ChirpTxStartTime': 0, 'ChirpRfFreqSlope': 47.95, 'ChirpRfFreqStart': 60}
        self.guiMonitor = {'pointCloud': 1, 'rangeProfile': 0, 'NoiseProfile': 0,
        ...
        self.measureRangeBiasAndRxChanPhase = {
            'enabled': 0, 'centerDist': 0, 'searchRange': 0}
        self.rangeRes = 0
        self.rangeAxisVals = np.zeros(
            int(self.chirpComnCfg['NumOfAdcSamples']/2))
        ...
        self.pointBounds = {'enabled': False, 'minX': 0,
                            'maxX': 0, 'minY': 0, 'maxY': 0, 'minZ': 0, 'maxZ': 0}
        ...
        self.Gradients = OrderedDict([
            ('bw', {'ticks': [(0.0, (0, 0, 0, 255)),
             (1, (255, 255, 255, 255))], 'mode': 'rgb'}),
            ('hot', {'ticks': [(0.3333, (185, 0, 0, 255)), (0.6666, (255, 220, 0, 255)), (
                1, (255, 255, 255, 255)), (0, (0, 0, 0, 255))], 'mode': 'rgb'}),
            ('jet', {'ticks': [(1, (166, 0, 0, 255)), (0.32247191011235954, (0, 255, 255, 255)), (0.11348314606741573, (0, 68, 255, 255)), (0.6797752808988764, (
                255, 255, 0, 255)), (0.902247191011236, (255, 0, 0, 255)), (0.0, (0, 0, 166, 255)), (0.5022471910112359, (0, 255, 0, 255))], 'mode': 'rgb'}),
            ('summer', {'ticks': [(1, (255, 255, 0, 255)),
             (0.0, (0, 170, 127, 255))], 'mode': 'rgb'}),
            ('space', {'ticks': [(0.562, (75, 215, 227, 255)), (0.087, (255, 170, 0, 254)), (0.332, (0, 255, 0, 255)), (
                0.77, (85, 0, 255, 255)), (0.0, (255, 0, 0, 255)), (1.0, (255, 0, 127, 255))], 'mode': 'rgb'}),
            ('winter', {'ticks': [(1, (0, 255, 127, 255)),
             (0.0, (0, 0, 255, 255))], 'mode': 'rgb'}),
            ('spectrum2', {
             'ticks': [(1.0, (255, 0, 0, 255)), (0.0, (255, 0, 255, 255))], 'mode': 'hsv'}),
            ('heatmap', {'ticks': [(1, (255, 0, 0, 255)),
             (0, (131, 238, 255, 255))], 'mode': 'hsv'})
        ])
        cmap = 'heatmap'
        ...
        self.predictedClassValue = ""  # Predition Class
        self.predictedClassConfidence = ""
        ...
        self.predictedClassLabel.setStyleSheet("font-size: 24px; color: black")

        self.confidenceLabel = QLabel(f"Confidence : {self.predictedClassConfidence}%")
        self.confidenceLabel.setStyleSheet("font-size: 24px; color: darkblue")
        ...
        if (demoName != ""):
            try:
                if (self.deviceType.currentText() in DEVICE_LIST[0:2]):
                    self.configType.setCurrentIndex(
                        x843_DEMO_TYPES.index(demoName))
                if (self.deviceType.currentText() in DEVICE_LIST[2:3]):
                    self.configType.setCurrentIndex(
                        x432_DEMO_TYPES.index(demoName))
                if (self.deviceType.currentText() in DEVICE_LIST[3:]):
                    self.configType.setCurrentIndex(
                        xWRL1432_DEMO_TYPES.index(demoName))
    ...
    def setPredictionValues(self, label, confidence):
        self.predictedClassValue = label
        self.predictedClassConfidence = f"{confidence:.2f}"

        # Update the QLabel widgets
        self.predictedClassLabel.setText(f"Prediction Class : {self.predictedClassValue}")
        self.confidenceLabel.setText(f"Confidence : {self.predictedClassConfidence}%")

        # print(f"[MainWindow] Updated Prediction: {self.predictedClassValue}, Confidence: {self.predictedClassConfidence}")
    ...
    def updateFallDetectionSensitivity(self):
        self.fallDetection.setFallSensitivity(((self.fallDetSlider.value(
        ) / self.fallDetSlider.maximum()) * 0.4) + 0.4)  # Range from 0.4 to 0.8
    ...
    def initFallDetectPane(self):
        ...
        self.fallDetSlider.valueChanged.connect(
            self.updateFallDetectionSensitivity)
        ...
        # Configure for Gesture
            ...
            if newConfig == DEMO_NAME_GESTURE:
                ...
                self.probabilityThresholds = [
                    0.99, 0.6, 0.6, 0.6, 0.6, 0.9, 0.9, 0.6, 0.6, 0.99]
                ...
                self.gestureList = ['  No Gesture   ',
                                    ...
                                    '     Shine     ']
                self.sumProbs = [0] * \
                    len(self.gestureList) * GESTURE_FEATURE_LENGTH
                self.gesture_featurePlots = {}
                self.gesture_featureVals = {
                    'dopplerAvgVals': [], 'rangeAvgVals': [], 'numPointsVals': []}
    ...
    def onChangeDeviceType(self):
        ...
        if (newDevice in DEVICE_LIST[0:2]):
            ...
            self.configType.addItems(x843_DEMO_TYPES)
            # DoubleCOMPort refers to xWRx843 parts
            self.parser.parserType = "DoubleCOMPort"
            self.configType.setCurrentIndex(-1)
            self.configType.currentIndexChanged.connect(
                self.onChangeConfigType)
            self.configType.setCurrentIndex(0)
        ...
        if (newDevice in DEVICE_LIST[2:3]):
            ...
            # SingleCOMPort refers to xWRLx432 parts
        ...
        for box in self.boundaryBoxes:
            ...
            try:
                ...
                if ('mpdArc' in box['name']):
                    ...
                    boxColor = pg.glColor(box['color'].itemData(
                        box['color'].currentIndex()))
    ...
    def initVitalsPlots(self):
        ...
        for i in range(MAX_VITALS_PATIENTS):
            ...
            if (self.vitalsIWRL6432 == 1):
                patientDict['plot'].setXRange(
                    0, NUM_VITALS_FRAMES_IN_PLOT_IWRL6432, padding=0.01)
                patientDict['plot'].setYRange(0, 120, padding=0.1)
                patientDict['plot'].getPlotItem().setLabel(
                    'left', 'Hear Rate and Breath Rate per minute')
                patientDict['plot'].getPlotItem().setLabel(
                    'bottom', 'Vital Signs Frame Number')
            else:
                patientDict['plot'].setXRange(
                    0, NUM_VITALS_FRAMES_IN_PLOT, padding=0.01)
            ...
            patientPaneLayout.addWidget(
                statusLabel, 0, 0, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                patientDict['status'], 1, 0, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                breathLabel, 0, 1, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                patientDict['breathRate'], 1, 1, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                heartLabel, 0, 2, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                patientDict['heartRate'], 1, 2, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
                rangeBinLabel, 0, 3, alignment=Qt.AlignHCenter)
            patientPaneLayout.addWidget(
    ...
    def init3dGraph(self):
        ...
        self.evmBox = gl.GLMeshItem(
            vertexes=verts, smooth=False, drawEdges=True, edgeColor=pg.glColor('r'), drawFaces=False)
    ...
    def initGesturePhysicalSetupPane(self):
        ...
        instructionsLabel.setText(
            "Stand 2m away, directly in front of the radar.")
    ...
    def initGestureTab(self):
        ...
        if self.configType.currentText() == DEMO_NAME_x432_GESTURE:
            ...
            self.gesture_featurePlots['dopplerAvgPlot'].setBackground(
                (70, 72, 79))
            self.gesture_featurePlots['dopplerAvgPlot'].showGrid(
                x=True, y=True)
            ...
            self.gesture_featurePlots['rangeAvgPlot'].setBackground(
                (70, 72, 79))
            ...
            self.gesture_featurePlots['numPointsPlot'].setBackground(
                (70, 72, 79))
    ...
    def initGroundSpeedTab(self):
        ...
        self.speedOutput.setStyleSheet(
            'background-color: rgb(70, 72, 79); color: white; font-size: 60px; font-weight: bold')
        self.speedOutputMph.setStyleSheet(
            'background-color: rgb(70, 72, 79); color: white; font-size: 60px; font-weight: bold')
        font = QFont()
        font.setPointSize(int(self.width() / 20))
        ...
        self.speedPlots['avgSpeedPlot'].plot(
            self.speedVals['speedVals'], pen=pen)
    ...
    def updateGestureFeatures(self, features):
        ...
        dopplerAvgData = collections.deque(
            self.gesture_featureVals['dopplerAvgVals'])
        # doppler avg feature is at index 1
        ...
        rangeAvgData = collections.deque(
            self.gesture_featureVals['rangeAvgVals'])
        ...
        numPointsData = collections.deque(
            self.gesture_featureVals['numPointsVals'])
        # num points feature is at index 4
        numPointsData.appendleft(features[4])
    ...
    def gesturePostProc(self, ann_probs):
        ...
        for i in range(numOutputProbs):
            if ann_probs[i] >= self.probabilityThresholds[i]:
                self.sumProbs[GESTURE_FEATURE_LENGTH *
                              numOutputProbs - numOutputProbs + i] = 1
            else:
                self.sumProbs[GESTURE_FEATURE_LENGTH *
                              numOutputProbs - numOutputProbs + i] = 0
    ...
    def updateGroundSpeedDisplay(self, text):
        self.speedOutput.setStyleSheet(
            f'background-color: blue; color: white; font-weight: bold')
    ...
    def initSurfaceClassificationTab(self):
        ...
        self.surfaceOutputRaw = QLabel(
            "<b>Grass Probability</b><br>0.0%", self)
    ...
    def surfaceHandler(self, classification):
        ...
        currentClassification = np.average(list(self.surfaceLatestResults)[
                                           0:5], weights=[5, 4, 3, 2, 1])
        ...
        self.surfaceOutputRaw.setText(
            "<b>Grass Classification Value</b><br>" + "{:8.5f}".format(classification * 100) + "%")
    ...
    def updateGraph(self, outputDict):
        ...
        if (velocity is not None):
            ...
            self.updateGroundSpeedMphDisplay(
                "{0:.2f} mph".format(velocity * 2.237))
        ...
        if (self.profile['elev_tilt'] != 0 or self.profile['az_tilt'] != 0):
            if (pointCloud is not None):
                for i in range(numPoints):
                    rotX, rotY, rotZ = eulerRot(
                        pointCloud[i, 0], pointCloud[i, 1], pointCloud[i, 2], self.profile['elev_tilt'], self.profile['az_tilt'])
                    pointCloud[i, 0] = rotX
                    pointCloud[i, 1] = rotY
                    pointCloud[i, 2] = rotZ
            if (tracks is not None):
                for i in range(numTracks):
                    rotX, rotY, rotZ = eulerRot(
                        tracks[i, 1], tracks[i, 2], tracks[i, 3], self.profile['elev_tilt'], self.profile['az_tilt'])
                    tracks[i, 1] = rotX
                    tracks[i, 2] = rotY
                    tracks[i, 3] = rotZ
            ...
            for box in self.boundaryBoxes:
                if ('mpdBox' in box['name']):
                    ...
                    if (boxIdx >= len(enhancedPresenceDet)):
                        print(
                            "Warning : Occupancy results for box that does not exist")
            ...
            for i in range(numPoints):
                if (i == 0):
                    ...
                    self.Peak1Magnitude = round(
                        np.log10(round((pointCloud[i, 4]*64 + pointCloud[i, 5]), 4)+1)*20, 1)
        ...
        if (OFF_CHIP_PRESENCE_DETECTION_ENABLED == 1 and self.profile['enabled'] == 1):
            if pointCloud.shape[0] > 0:
                DBSCANObj = DBSCAN(eps=self.profile['maxDistance'], min_samples=self.profile['minPoints']).fit(
                    pointCloud[:, 0:3])
                clusters = set(DBSCANObj.labels_)  # Set of labels (no repeats)
                # List of labels by points (with repeated labels)
                pointLabels = DBSCANObj.labels_
                centroids = []
                # No repeats in a cluster
                for clusterIdx, cluster in enumerate(clusters):
                    if (cluster >= 0):  # Discard the -1 cluster which is the unassociated points
                        centroids.append(
                            {'x': 0, 'y': 0, 'z': 0, 'snr': 0, 'numPoints': 0, 'r': 0, 'theta': 0})
                        for labelIdx, label in enumerate(pointLabels):
                            if (label == cluster):
                                # X
                                centroids[clusterIdx]['x'] = centroids[clusterIdx]['x'] + \
                                    pointCloud[labelIdx][0]
                                # Y
                                centroids[clusterIdx]['y'] = centroids[clusterIdx]['y'] + \
                                    pointCloud[labelIdx][1]
                                # Z
                                centroids[clusterIdx]['z'] = centroids[clusterIdx]['z'] + \
                                    pointCloud[labelIdx][2]
                                # SNR
                                centroids[clusterIdx]['snr'] = centroids[clusterIdx]['snr'] + \
                                    pointCloud[labelIdx][4]
                                # store the count to divide later
                                centroids[clusterIdx]['numPoints'] = centroids[clusterIdx]['numPoints'] + 1
                        ...
                        centroids[clusterIdx]['z'] = centroids[clusterIdx]['z'] / \
                            centroids[clusterIdx]['numPoints']
                        centroids[clusterIdx]['r'] = math.sqrt(math.pow(
                            centroids[clusterIdx]['x'], 2) + math.pow(centroids[clusterIdx]['y'], 2))
                        if (float(centroids[clusterIdx]['y']) > 0):
                            centroids[clusterIdx]['theta'] = math.atan(float(
                                centroids[clusterIdx]['x']) / float(centroids[clusterIdx]['y'])) * 180 / np.pi
                        else:
                    ...
                    for machine in majorMotionStateMachines:
                        ...
                        # Boundary Box index may not equal the state machine index
                        bbIndex = machine.getBoundaryArcIndex()
                        # Update boundary box colors based on results of Motion/Presence Detection
                        if (state == 1):
                            self.boundaryBoxes[bbIndex]['color'].setCurrentText(
                                'Red')
                        if (state == 0):
                            self.boundaryBoxes[bbIndex]['color'].setCurrentText(
                                'Blue')
            ...
            if (patientId < self.profile['maxTracks']):
                ...
                if (self.vitalsIWRL6432 == 1):
                    ...
                    while (len(self.vitalsPatientData[patientId]['heartWaveform']) > NUM_VITALS_FRAMES_IN_PLOT_IWRL6432):
                        self.vitalsPatientData[patientId]['heartWaveform'].pop(
                            0)
                    ...
                    while (len(self.vitalsPatientData[patientId]['breathWaveform']) > NUM_VITALS_FRAMES_IN_PLOT_IWRL6432):
                        self.vitalsPatientData[patientId]['breathWaveform'].pop(
                            0)
                else:
        ...
        if (self.displayFallDet.checkState() == 2):
            ...
            if (heights is not None):
                ...
                fallDetectionDisplayResults = self.fallDetection.step(
                    heights, tracks)
            ...
            # For 3D People Tracking and vitals demos, the tracks and track indexes come one frame after the associated point cloud
                ...
                if (self.previousClouds[len(self.previousClouds) - 1].shape[0] != trackIndexs.shape[0]):
                    ...
                    if (self.lastFrameErrorFlag == False):
                        print("Warning in gui_main.py: number of points in last frame (" + str(self.previousClouds[len(
                            self.previousClouds) - 1].shape[0]) + ") does not match number of track indexes (" + str(trackIndexs.shape[0]) + ")")
                else:
                    self.previousClouds[len(
        ...
        if (classifierOutput is not None):
            ...
            for trackNum, trackName in enumerate(tracks):
                ...
                numUnknownTags = sum(
                    1 for i in self.classifierTags[trackID] if i == 0)
                ...
                if (numUnknownTags > MAX_NUM_UNKNOWN_TAGS_FOR_HUMAN_DETECTION or sumOfTags == 0):
                    # Target was NOT detected to be human in the current frame, save for next frame
                ...
                # If we have enough tags and the majority of them are for human, then detect human
                    ...
                    # Target WAS detected to be human in the current frame, save for next frame
                ...
                self.classifierStr[trackID].setZ(trackName[3] + 0.1)
            ...
            tracksToShuffle = set(
                self.tracksIDsInPreviousFrame) - set(trackIDsInCurrFrame)
            for track in tracksToShuffle:
                for frame in range(TAG_HISTORY_LEN):
                    ...
                    self.classifierTags[track].appendleft(0)
                # Since target was not detected in current frame, reset the wasTargetHuman flag
                self.wasTargetHuman[trackID] = 0
        ...
        if (self.graphTabs.currentWidget() == self.pcplot):
            ...
            if (self.graphFin):
                ...
                self.graphFin = 0
                self.get_thread = updateQTTargetThread3D(cumulativeCloud, tracks, self.scatter, self.pcplot, numTracks, self.ellipsoids, self.coordStr, classifierOutput,
                                                         self.zRange, self.colorGradient, self.pointColorMode.currentText(), self.plotTracks.isChecked(), self.trackColorMap, self.pointBounds)
            ...
            if (self.parser.parserType == "DoubleCOMPort"):  # Range plot not supported on 6843
                ...
                if (rangeProfileMajor is not None):
                    ...
                    if (numRangeBinsParsed == next_power_of_2(round(self.chirpComnCfg['NumOfAdcSamples']/2))):
                        ...
                        # Update graph data
                        self.rangeData.setData(
                            self.rangeAxisVals, rangeProfileMajor)
                    else:
                        print(
                ...
                if (rangeProfileMinor is not None):
                    ...
                    if (numRangeBinsParsed == next_power_of_2(round(self.chirpComnCfg['NumOfAdcSamples']/2))):
                        ...
                        # Update graph data
                        self.rangeData.setData(
                            self.rangeAxisVals, rangeProfileMinor)
                    else:
                        print(
            ...
            if (self.parser.parserType == "DoubleCOMPort"):  # Range plot not supported on 6843
                ...
                if (rangeProfileMajor is not None):
                    ...
                    if (numRangeBinsParsed == next_power_of_2(round(self.chirpComnCfg['NumOfAdcSamples']/2))):
                        ...
                        highlight_x = [self.rangeAxisVals[i]
                                       for i in highlight_indices]
                        highlight_y = [rangeProfileMajor[i]
                                       for i in highlight_indices]
                        data = [{'pos': (x_val, y_val)} for x_val, y_val in zip(
                            highlight_x, highlight_y)]
                        self.HighlightPlotPeak1.setData(data)

                        # Adding labels to highlighted points
                        for i in range(len(highlight_indices)):
                            self.peakLabel1.setPos(
    ...
    def updateNumTracksBuffer(self):
        ...
        self.classifierTags = [deque([0] * TAG_HISTORY_LEN, maxlen=TAG_HISTORY_LEN)
    ...
    def parseCfg(self, fname):
        ...
        for line in self.cfg:
            ...
            if (len(args) > 0):
                ...
                # trackingCfg
                    ...
                    self.trackColorMap = get_trackColors(
                        self.profile['maxTracks'])
                    ...
                    topZ = float(args[6])
                    self.addBoundBox('trackerBounds', leftX,
                                     rightX, nearY, farY, bottomZ, topZ)
                ...
                # Only used for Small Obstacle Detection
                    ...
                    if (len(args) < 8):
                        print(
                            "Error: mpdBoundaryBox had fewer arguments than expected")
                        continue
                    ...
                    if self.mpdZoneType is None:
                        ...
                        print(
                            "Error: Cannot add mpdBoundaryBox when one or more %s is already defined." % self.mpdZoneType)
                        continue
                    else:
                    ...
                    if (len(args) < 8):
                        print(
                            "Error: mpdBoundaryArc had fewer arguments than expected")
                        continue
                    ...
                    if self.mpdZoneType is None:
                        ...
                        print(
                            "Error: Cannot add mpdBoundaryArc when one or more %s is already defined." % self.mpdZoneType)
                        continue
                    else:
                        print("mpdZoneType: %s" % self.mpdZoneType)
                    ...
                    if (OFF_CHIP_PRESENCE_DETECTION_ENABLED == 1):
                        majorMotionStateMachines.append(
                            majorBoundaryArcStateMachineType())
                    self.addBoundBox(name, minR, maxR,
                                     minTheta, maxTheta, minZ, maxZ)
                    ...
                    try:
                        ...
                        self.chirpComnCfg['ChirpRampEndTime'] = 10 * \
                            float(args[6])
                    ...
                    if (len(args) < 6):
                        print(
                            "Error: chirpTimingCfg had fewer arguments than expected")
                        continue
                    self.chirpTimingCfg['ChirpIdleTime'] = 10.0 * \
                        float(args[1])
                    self.chirpTimingCfg['ChirpAdcSkipSamples'] = int(
                        args[2]) << 10
                    self.chirpTimingCfg['ChirpTxStartTime'] = 10.0 * \
                        float(args[3])
                ...
                # NOTE - Only major mode is supported
                    ...
                    if (len(args) < 9):
                        print(
                            "Error: majorStateCfg had fewer arguments than expected")
                        continue
                        ...
                        for box in self.boundaryBoxes:
                            if ('mpdArc' in box['name']):
                                majorMotionStateMachines[stateMachineIdx].configure(pointThre1, pointThre2, snrThre2,
                                                                                    ...
                                                                                    float(box['boundList'][0].text()), float(
                                                                                        box['boundList'][1].text()),
                                                                                    float(box['boundList'][2].text()), float(
                                                                                        box['boundList'][3].text()),
                ...
                # This is specifically guiMonitor for 60Lo, this parsing will break the gui when an SDK 3 config is sent
                    ...
                    if (self.deviceType.currentText() in DEVICE_LIST[2] or self.deviceType.currentText() in DEVICE_LIST[3]):
                        if (len(args) < 12):
                            print(
                                "Error: guiMonitor had fewer arguments than expected")
                            continue
                    ...
                    self.measureRangeBiasAndRxChanPhase['centerDist'] = float(
                        args[2])
                    self.measureRangeBiasAndRxChanPhase['searchRange'] = float(
                        args[3])
                    ...
                    self.sigProcChain['minorMotionEnabled'] = 1 if int(
                        args[3]) > 1 else 0
        ...
        if (self.measureRangeBiasAndRxChanPhase['enabled'] == 1):
            ...
            if (self.channelCfg['TX'] != 3):
                print(
                    "Error : measureRangeBiasAndRxChanPhase requires 2 TX enabled. \n Change the 2nd argument of channelCfg to 3")
            if (self.channelCfg['RX'] != 7):
                print(
                    "Error : measureRangeBiasAndRxChanPhase requires 3 RX enabled. \n Change the 1st argument of channelCfg to 7")

            # Create range bin zone in the gui
            rangeMin = self.measureRangeBiasAndRxChanPhase['centerDist'] - \
                self.measureRangeBiasAndRxChanPhase['searchRange']/2
            rangeMax = self.measureRangeBiasAndRxChanPhase['centerDist'] + \
                self.measureRangeBiasAndRxChanPhase['searchRange']/2
            self.compRangeBiasZone = pg.LinearRegionItem(
                (rangeMin, rangeMax), movable=False, span=(0, 0.94))
            self.rangePlot.addItem(self.compRangeBiasZone)
            text = pg.TextItem(text='Place Peak in Calibration Zone', color=(
                200, 0, 0), anchor=(0.5, 0))
        ...
        self.rangeRes = (3e8*(100/self.chirpComnCfg['DigOutputSampRate'])*1e6)/(
            2*self.chirpTimingCfg['ChirpRfFreqSlope']*1e12*self.chirpComnCfg['NumOfAdcSamples'])
        self.rangePlot.setXRange(
